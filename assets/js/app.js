...
Hooks.PlaybackProgress = {
  ...
  start() {
    ...
    const bar = this.el.querySelector("[data-role='progress-bar']")
    if (!bar) {
      return
    }

    const duration = parseInt(this.el.dataset.durationMs || "", 10)
    const startedAt = parseInt(this.el.dataset.startedAt || "", 10)

    if (!Number.isFinite(duration) || duration <= 0 || !Number.isFinite(startedAt) || startedAt <= 0) {
      bar.style.width = "33%"
