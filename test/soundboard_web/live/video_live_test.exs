defmodule SoundboardWeb.VideoLiveTest do
  ...
  test "renders the retro broadcast console", %{conn: conn} do
    conn = put_session(conn, :theme, "retro")

    {:ok, view, html} = live(conn, "/video")

    assert html =~ ~s(data-theme="retro")
    assert has_element?(view, ".riso-video-deck")
    assert has_element?(view, ".riso-video-load input[name='url']")
    assert has_element?(view, "#shared-video-player.riso-video-screen")
    assert has_element?(view, ".riso-video-queue-empty")
  end
