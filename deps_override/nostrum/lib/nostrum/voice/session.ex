defmodule Nostrum.Voice.Session do
  ...
  def handle_info({:udp, _erl_port, _ip, _port, packet}, state) do
    case packet do
      # Skip all RTCP packets (types 200-207)
      <<2::2, _::1, _::5, pt::8, _rest::binary>> when pt >= 200 and pt <= 207 ->
        :noop
    ...
    %MatchError{term: :error} ->
      # Decrypt returned :error - silently skip (likely keep-alive or unsupported format)
      {:noreply, state}
    error ->
